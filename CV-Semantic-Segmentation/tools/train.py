...
def main():
    ...
    distributed = False #args.local_rank >= 0
    if distributed:
    ...
    test_dataset = eval('datasets.'+config.DATASET.DATASET)(
                        ...
                        crop_size=test_size,
                        downsample_rate=1)
    test_dataset.class_weights = None
    ...
    if config.TRAIN.RESUME:
        ...
        for filename in os.listdir(final_output_dir):
            if 'checkpoint_' in filename:
                epochArray = [int(s) for s in re.findall(r'\d+', filename)]
                if len(epochArray) == 1:
                    if epochArray[0] > highestEpochNumber:
                        loadCheckpoint = filename
                        highestEpochNumber = epochArray[0]
        
        if highestEpochNumber == 0:
            loadCheckpoint = 'checkpoint.pth.tar'
            print("No recent checkpoint found, try just checkpoint")
        else:
            print("Most recent checkpoint found is " + str(highestEpochNumber))


        model_state_file = os.path.join(final_output_dir, loadCheckpoint)
    ...
    for epoch in range(last_epoch, end_epoch):
        ...
        logger.info('=> saving checkpoint to {}'.format(final_output_dir + 'checkpoint_'+ str(epoch) +'.pth.tar'))
        torch.save({
                'epoch': epoch,
                'best_mIoU': best_mIoU,
                'state_dict': model.module.state_dict(),
                'optimizer': optimizer.state_dict(),
            }, os.path.join(final_output_dir,'checkpoint_'+ str(epoch) +'.pth.tar'))
        epochTime = time.time() - startEpoch
        print("Epoch took " + str(round(epochTime, 2))+ "s")
        ...
        if valCounter >=20:
            ...
            print("Validation took " + str(round(time.time() - startEpoch-epochTime, 2))+ "s")
        valCounter +=1
        startEpoch = time.time()
